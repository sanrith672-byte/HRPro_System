// ============================================================
// HR Pro Multi-Company API — Cloudflare Workers + D1
// ============================================================
...
async function handleRequest(request, env) {
  ...
  try {
    ...
    // Generic tables with company_id
    if (path==='/overtime')         { if(m==='GET') return getAllCo('overtime ot JOIN employees e ON ot.employee_id=e.id','ot.*,e.name as employee_name','ot.created_at DESC','ot',request,env,url); if(m==='POST') return insCo(request,env,url,'overtime',['employee_id','date','hours','rate','pay','reason','status']); }
    ...
    if (path==='/allowances')       { if(m==='GET') return getAllCo('allowances al JOIN employees e ON al.employee_id=e.id','al.*,e.name as employee_name','al.created_at DESC','al',request,env,url); if(m==='POST') return insCo(request,env,url,'allowances',['employee_id','type','amount','month','note']); }
    if (path.match(/^\/allowances\/\d+$/))  { const id=+path.split('/')[2]; if(m==='DELETE') return delCo(id,'allowances',request,env,url); }
    if (path==='/loans')            { if(m==='GET') return getAllCo('loans ln JOIN employees e ON ln.employee_id=e.id','ln.*,e.name as employee_name','ln.created_at DESC','ln',request,env,url); if(m==='POST') return insCo(request,env,url,'loans',['employee_id','amount','loan_date','due_date','note','paid_amount','status','installment_months','installment_amount']); }
    ...
    if (path==='/expenses')         { if(m==='GET') return getAllCo('expense_requests er JOIN employees e ON er.employee_id=e.id','er.*,e.name as employee_name','er.created_at DESC','er',request,env,url); if(m==='POST') return insCo(request,env,url,'expense_requests',['employee_id','category','amount','request_date','description','status']); }
    ...
    if (path==='/leave')            { if(m==='GET') return getAllCo('leave_requests lr JOIN employees e ON lr.employee_id=e.id','lr.*,e.name as employee_name','lr.created_at DESC','lr',request,env,url); if(m==='POST') return insCo(request,env,url,'leave_requests',['employee_id','leave_type','start_date','end_date','days','reason','status']); }
...
async function getCompanies(env) {
  try {
    await env.DB.prepare(`CREATE TABLE IF NOT EXISTS companies (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, code TEXT UNIQUE NOT NULL, logo_url TEXT DEFAULT '', address TEXT DEFAULT '', phone TEXT DEFAULT '', email TEXT DEFAULT '', website TEXT DEFAULT '', created_at TEXT)`).run();
...
async function getCompany(id, env) {
  const c = await env.DB.prepare('SELECT * FROM companies WHERE id=?').bind(id).first();
...
async function updateCompany(id, request, env) {
  const b = await request.json();
  await env.DB.prepare(`UPDATE companies SET name=?,code=?,logo_url=?,address=?,phone=?,email=?,website=? WHERE id=?`).bind(b.name||'',b.code||'',b.logo_url||'',b.address||'',b.phone||'',b.email||'',b.website||'',id).run();
  return json({message:'Updated'});
}
async function deleteCompany(id, env) {
...
async function deleteEmp(id, request, env, url) {
  const co=cid(request,url,null);
  await env.DB.prepare('DELETE FROM employees WHERE id=? AND company_id=?').bind(id,co).run();
  return json({message:'Deleted'});
}
async function saveMedia(id, col, request, env) {
  const b=await request.json();
  await env.DB.prepare(`UPDATE employees SET ${col}=? WHERE id=?`).bind(b.data||'',id).run();
  return json({message:'Saved'});
}
async function delMedia(id, col, env) {
...
async function updateDept(id, request, env) {
  const b=await request.json();
  await env.DB.prepare(`UPDATE departments SET name=?,manager=?,icon=?,color=?,updated_at=datetime('now') WHERE id=?`).bind(b.name||'',b.manager||'',b.icon||'🏢',b.color||'#118AB2',id).run();
  return json({message:'Updated'});
}
async function delDept(id, env) {
  const c=await env.DB.prepare('SELECT COUNT(*) as c FROM employees WHERE department_id=?').bind(id).first();
  if(c?.c>0) return err('មានបុគ្គលិកក្នុងនាយកដ្ឋាននេះ',400);
...
async function updAtt(id, request, env) {
  const b=await request.json();
  await env.DB.prepare(`UPDATE attendance SET check_in=?,check_out=?,status=?,notes=? WHERE id=?`).bind(b.check_in||'',b.check_out||'',b.status||'present',b.notes||'',id).run();
  return json({message:'Updated'});
}
...
async function updSal(id, request, env) {
  const b=await request.json(); const net=(b.base_salary||0)+(b.bonus||0)-(b.deduction||0);
  await env.DB.prepare(`UPDATE salary_records SET base_salary=?,bonus=?,deduction=?,net_salary=?,status=?,note=?,updated_at=datetime('now') WHERE id=?`).bind(b.base_salary||0,b.bonus||0,b.deduction||0,net,b.status||'pending',b.note||'',id).run();
  return json({message:'Updated'});
}
async function paySal(id, env) {
  await env.DB.prepare(`UPDATE salary_records SET status='paid',updated_at=datetime('now') WHERE id=?`).bind(id).run();
  return json({message:'Paid'});
}
...
async function updRec(id, request, env, table) {
  const b=await request.json(); const keys=Object.keys(b).filter(k=>k!=='company_id');
  if(!keys.length) return json({message:'No changes'});
  await env.DB.prepare(`UPDATE ${table} SET ${keys.map(k=>`${k}=?`).join(',')},updated_at=datetime('now') WHERE id=?`).bind(...keys.map(k=>b[k]),id).run();
  return json({message:'Updated'});
}
async function delRec(id, env, table) {
  await env.DB.prepare(`DELETE FROM ${table} WHERE id=?`).bind(id).run();
  return json({message:'Deleted'});
}
async function delCo(id, table, request, env, url) {
  const co=cid(request,url,null);
  await env.DB.prepare(`DELETE FROM ${table} WHERE id=? AND company_id=?`).bind(id,co).run();
  return json({message:'Deleted'});
}
async function repay(id, request, env) {
  const {amount}=await request.json();
  const loan=await env.DB.prepare('SELECT * FROM loans WHERE id=?').bind(id).first();
  if(!loan) return err('Not found',404);
  const paid=(loan.paid_amount||0)+amount; const status=paid>=loan.amount?'paid':'active';
  await env.DB.prepare(`UPDATE loans SET paid_amount=?,status=?,updated_at=datetime('now') WHERE id=?`).bind(paid,status,id).run();
  return json({message:'Repayment recorded',paid_amount:paid,status});
}
...
async function initDB(env) {
  ...
  const migs=[
    `ALTER TABLE departments ADD COLUMN company_id INTEGER DEFAULT 1`,
    ...
    `ALTER TABLE salary_records ADD COLUMN company_id INTEGER DEFAULT 1`,
    `ALTER TABLE overtime ADD COLUMN company_id INTEGER DEFAULT 1`,
    `ALTER TABLE allowances ADD COLUMN company_id INTEGER DEFAULT 1`,
    `ALTER TABLE loans ADD COLUMN company_id INTEGER DEFAULT 1`,
    `ALTER TABLE expense_requests ADD COLUMN company_id INTEGER DEFAULT 1`,
    `ALTER TABLE general_expenses ADD COLUMN company_id INTEGER DEFAULT 1`,
    `ALTER TABLE leave_requests ADD COLUMN company_id INTEGER DEFAULT 1`,
    `ALTER TABLE app_config ADD COLUMN company_id INTEGER DEFAULT 1`,
  ...
  await Promise.allSettled(migs.map(m=>env.DB.prepare(m).run()));
  // Default company
  try {
